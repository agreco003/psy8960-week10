...
gss_tbl <- tibble(gss_data) %>%
  ...
  select(which(colMeans(is.na(gss_data)) <= 0.25), workhours) #selecting all columns WHICH have colmeans of missing data less or equal to 25%. #check with colMeans(is.na(gss_tbl)) shows missingness all less than .25. Also selected workhours manually as it isn't included in the gss_data originally. 
